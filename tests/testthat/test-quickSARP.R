...
########
# Note: quickSARP is a simple function that runs all of the other functions 
...
test_that("When given an improper geometry statement, quickSARP will 
          throw an error", {
...
test_that("When quickSARP gets to the findLand statement, expect a message", {
 expect_message(quickSARP(query, rank, limit = 100, geometry = geometry, 
...
test_that("quickSARP returns a list", {
 expect_type(quickSARP(query, rank, limit = 100, geometry = geometry, 
