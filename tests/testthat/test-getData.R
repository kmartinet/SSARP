...
test_that("The getData function returns a dataframe when geometry = NULL", {
  expect_s3_class(getData(key = key, limit = 100, geometry = NULL), 
                  "data.frame")
})

test_that("The getData function returns a dataframe with 
          valid geometry statement", {
  expect_s3_class(getData(key = key, limit = 100, 
